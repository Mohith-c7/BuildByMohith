      ...
      <section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {Object.entries(INITIAL_STATS).map(([key, value], i) => (
            <MetricOverlay key={key} label={key.toUpperCase()}>
              <motion.div
      ...
      <section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {SERVICES.map((service, i) => (
            <MetricOverlay key={service.id} label={service.name.toUpperCase()}>
              <motion.div
                initial={{ opacity: 0, x: -20 }}
                animate={{ opacity: 1, x: 0 }}
                transition={{ delay: 0.4 + i * 0.1 }}
                className="bg-black/40 border border-emerald-900/30 p-5 rounded-lg hover:border-emerald-500/30 transition-all group h-full"
              >
